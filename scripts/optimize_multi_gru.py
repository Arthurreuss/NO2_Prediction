...
def build_context():
    ...
    print(f"[optimize_multigru] Using device: {device}")
...
def objective(trial: optuna.Trial, ctx: dict) -> float:
    ...
    Optuna objective:
    - sample hyperparams
    - train MultiGRU with early stopping
    ...
    best_model_path = results_dir / f"multigru_best_trial_{trial.number}.pt"
    best_val_rmse_no2 = float("inf")
    epochs_no_improve = 0

    experiment_name = "MultiGRU_Optimization_Optuna_h72"
    mlflow.set_experiment(experiment_name)

    with mlflow.start_run(run_name=f"MultiGRU_optuna_t{trial.number}"):
        mlflow.log_param("model_type", "MultiGRU")
...
def main():
    ...
    study_name = opt_cfg.get("study_name", "multigru_optuna_study")
